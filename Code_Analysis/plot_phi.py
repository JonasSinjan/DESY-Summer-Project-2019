...
ny=n
nz=n
filename= r"/lustre/fs23/group/that/jonas/Github_repo/DESY/phi0init/Runs/512_test/PHI0.BIN"
print(filename)  
fd = open(filename, 'rb')
...
dir_data = r"/lustre/fs23/group/that/jonas/Github_repo/DESY/3d_disp_mem/Runs/512_amp1/"
...
z=[phi0_512[:,:,slice_index], phi_amp05[:,:,slice_index], phi_amp1[:,:,slice_index], phi_amp1[:,:,slice_index]]#one slice
...
ax1.set_title('3D PHI AMP05', fontsize=14)
fig = plt.gcf()
plt.clim()   # clamp the color limits
plt.colorbar()
...
ax2.set_title('3D PHI AMP1', fontsize=14)
fig = plt.gcf()
plt.clim()   # clamp the color limits
plt.colorbar()
